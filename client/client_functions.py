...
def log_in_to_server(username, password, server_url):
    ...
    if os.path.exists("token.json"):
        ...
        response = requests.get(
            f"{server_url}api/v1/token-test", headers=headers, timeout=10
        )
        ...
        login_response = requests.post(
            server_url, data=payload, headers=login_headers, timeout=10
        )
...
def get_account_info(server_url: str) -> None:
    ...
    headers = {"Authorization": f"Bearer {get_token()}"}
    response = requests.get(
        f"{server_url}{account_url_suffix}", headers=headers, timeout=10
    )
    data = response.json()
    try:
...
def get_sym_key(server_url: str, password: str, friend_username: str):
    ...
    payload = {"user_password": password, "friend_username": friend_username}

    response = requests.post(
        f"{server_url}{account_url_suffix}", json=payload, headers=headers, timeout=10
    )
    data = response.json()
    return data
...
def register_user(
    ...
    response = requests.post(
        f"{server_url}{account_url_suffix}", json=payload, timeout=10
    )
...
def add_user_friends(server_url: str, friend_username: str):
    ...
    response = requests.post(
        f"{server_url}{account_url_suffix}{friend_username}",
        headers=headers,
        timeout=10,
    )
...
def get_thoughts_for_user(server_url: str, username: str) -> None:
    ...
    headers = {"Authorization": f"Bearer {get_token()}"}
    response = requests.get(
        f"{server_url}{account_url_suffix}/{username}", headers=headers, timeout=10
    )
    data = response.json()
...
def wrap_encrypt_sym_key(
    sym_key: bytes, server_url: str, friend_username: Union[str, None] = None
) -> Union[str, bytes]:
    ...
    if friend_username:
        public_key = serialization.load_pem_public_key(
            get_public_key_friend(server_url, friend_username).encode("utf-8")
        )  # ==To change to use the endpoint, drop the username
        # Encrypt the symmetric key with the client's public key
        encrypted_sim_key = public_key.encrypt(
            sym_key,
            padding.OAEP(
        ...
        public_key = serialization.load_pem_public_key(
            get_public_key(server_url).encode("utf-8")
        )  # ==To change to use the endpoint, drop the username
        # Encrypt the symmetric key with the client's public key
        encrypted_sim_key = public_key.encrypt(
            sym_key,
