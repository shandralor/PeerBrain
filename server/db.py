...
def confirm_registration_token(user_key: str) -> None:
    ...
    update = {"disabled": False, "confirmation_token": "verified"}
    try:
...
def create_password_reset_token(username: str, email: str) -> None:
    ...
    password_reset_object = {"key": username, "reset_token": reset_token}
...
def add_friend(username, friend_username):
    ...
    try:
        ...
        if not friend_username in get_users():
            ...
            return (
                USERS.update(update, user_key),
                f"User {friend_username} added as a friend successfully!",
            )
    except Exception as error_message:
        logging.exception(error_message)
        return None
...
def send_keys_to_remote_server(
    public_key: str, symmetric_key: str, username: str, hashed_password: str
) -> Union[bool, None]:
    ...
    user_key_store = {
        ...
        "key_store": {"public_key": public_key, "symmetric_key": symmetric_key},
    }
...
def get_encrypted_sym_key(username: str, user_password, friend_username: str):
    ...
    response = requests.post(
        f"{url}{url_suffix}", headers=headers, json=encrypted_sym_request
    )
    data = response.json()
    if response.status_code == 200:
