...
from db import (
    ...
    create_user,
    gen_pw_hash,
    get_encrypted_sym_key,
    get_friends_by_username,
    get_thoughts,
    get_user_by_email,
    get_user_by_username,
    get_users,
    send_keys_to_remote_server,
...
async def reset_user_password(username: str, token: str):
    ...
    if password_token_object and password_token_object["reset_token"] == token:
        ...
        raise HTTPException(
            status_code=400, detail="Invalid/expired password reset token!"
        )
...
async def get_thoughts_for_user(
...
async def get_thought(
    query_str: str, current_user: User = Depends(get_current_active_user)
):
    """
...
async def add_friends(
    friend_username: str, current_user: User = Depends(get_current_active_user)
):
    """
...
async def create_new_thought(
    thought: Thought, current_user: User = Depends(get_current_active_user)
):
    """
...
async def post_keystore_user(
    keystore: KeyStore, current_user: User = Depends(get_current_active_user)
):
    """
...
async def get_user_key_store(
    sym_key_req: SymKeyRequest, current_user: User = Depends(get_current_active_user)
):
    """
